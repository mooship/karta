...
/**
 * Every `@media (max-width: ...)` breakpoint declared across this
 * package's own stylesheets, alongside the file it came from.
 ...
 *   references to `MOBILE_BREAKPOINT_PX` — this scan is what actually
 *   guards against one of them silently drifting from that shared constant
 *   (see `useIsDesktopViewport.ts`, `@karta/react`) instead of a code
 *   comment alone.
 */
