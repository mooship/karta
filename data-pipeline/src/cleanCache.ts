...
/** Parses `pnpm run cache:clean`'s mode: `"all"` for `--all`, `"stale"` otherwise. */
...
/**
 * `pnpm run cache:clean` entry point: deletes the whole cache directory in
...
/* v8 ignore start -- exercised via `pnpm run cache:*`, not unit tests */
